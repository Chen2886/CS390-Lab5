...
def buildGenerator():
    ...
    model.add(keras.layers.Dense(IH * IW, input_dim=NOISE_SIZE))
    model.add(keras.layers.LeakyReLU(alpha=0.2))
    ...
    model.add(keras.layers.Reshape(IMAGE_SHAPE))
    
    # Creating a Keras Model out of the network
    inputTensor = Input(shape = (NOISE_SIZE,))
    return Model(inputTensor, model(inputTensor))

def buildGAN(images, epochs = 40000, batchSize = 32, loggingInterval = 10):
    # Setup
    opt = Adam(lr = 0.0002)
